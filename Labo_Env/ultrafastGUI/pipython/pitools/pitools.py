...
__signature__ = 0x84a1dd4ea97fe22f5271064ca9115f0
...
def stopall(pidevice, **kwargs):
    ...
    for GCS21 Controllers: Stops all axes an waits until the affected axes have finished their stop procedure.
    :type pidevice : pipython.gcscommands.GCSCommands
    :param kwargs : Optional arguments with keywords that are passed to sub functions.
    :return: Only for GCS21 Controllers returns the affected axes
    :rtype: list. Only for GCS21 Controllers.
    """
    if isdeviceavailable([GCS2Commands, ], pidevice):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21Tools(pidevice).stopall(**kwargs)
