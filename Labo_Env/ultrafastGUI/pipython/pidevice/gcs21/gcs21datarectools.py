...
from pipython.pidevice.gcs21.gcs21commands_helpers import (
    PIBlockNames,
    PIBlockKeys,
    PIMemoryTypeKeys,
    PIDataRecorderKeys,
...
__signature__ = 0x730f8eaebe3ccda7b6ddff63f608e0f5

# Class inherits from object, can be safely removed from bases in python3 pylint: disable=R0205
# Too many instance attributes pylint: disable=R0902
class GCS21Datarecorder(object):
    """Set up and use the data recorder of a GCS21device."""
    ...
    def number_of_values(self, value):
        ...
        debug('GCS21Datarecorder.number_of_values: set to', self._record_rate)

    @number_of_values.deleter
    def number_of_values(self):
        """Resets the number of values which should be read per trace."""
        self._number_of_values = None
        debug('GCS21Datarecorder.number_of_values: reset')
    ...
    def offset(self, value):
        ...
        debug('GCS21Datarecorder.offset: set to', self._offset)

    @offset.deleter
    def offset(self):
        """Resets the offset for start reading the recorded values."""
        self._offset = None
        debug('GCS21Datarecorder.offset: reset')
    ...
    def timeout(self, value):
        ...
        debug('GCS21Datarecorder.timeout: set to', self._timeout)

    @timeout.deleter
    def timeout(self):
        """Resets the timeout in seconds for reading the recorded values."""
        self._timeout = None
        debug('GCS21Datarecorder.timeout: reset')
    ...
    def record_rate(self, value):
        ...
        debug('GCS21Datarecorder.record_rate: set to', self._record_rate)

    @record_rate.deleter
    def record_rate(self):
        """Reset record rate of the data recorder."""
        self._record_rate = None
        debug('GCS21Datarecorder.record_rate: reset')
    ...
    def traces(self, value):
        ...
        debug('GCS21Datarecorder.traces: set to %r', self._traces)#

    @traces.deleter
    def traces(self):
        """Resets the current trace configuration."""
        self._traces = None
        debug('GCS21Datarecorder.traces: reset')
    ...
    def trigger(self, value):
        ...
        debug('GCS21Datarecorder.trigger: set to %r', self._trigger)

    @trigger.deleter
    def trigger(self):
        """Resets the current trigger configuration."""
        self._trigger = None
        debug('GCS21Datarecorder.trigger: reset')
    ...
    def _read_configurable_triggers_and_configurable_trigger_options(self):
        response = self._gcs.qUSG('CMD REC TRG')
        self._parse_qusg_cmd_rec_trg_response(response)

    def _parse_qusg_cmd_rec_trg_response(self, usg_cmd_rec_trg_response):
        try:
            command_overview_dict = \
                usg_cmd_rec_trg_response[0][PIBlockNames.COMMAND_OVERVIEW.value][0]
            recorder_trigger_subblock = find_subblock_with_key(
                command_overview_dict, PIBlockKeys.RECORDER_TRIGGER.value)
            ...
            raise ValueError('Unexpected format of response on "USG? CMD REC TRG"') from exception
