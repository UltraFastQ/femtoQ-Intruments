...
from .gcs21commands_helpers import PIAxisStatusKeys, PIValueDataTypes, PIBlockKeys, PIBlockNames, PIContainerUnitKeys, \
...
__signature__ = 0x960dac694c5d087514627b752d42467b
...
class GCS21Commands(GCSBaseCommands):
    ...
    def _init_settings(self):
        self._settings = {'paramconv': {}, 'validucls': [], 'paramconv_ucl': ''}
    ...
    def clearparamconv(self):
        ...
        debug('GCS21Commands.clearparamconv()')
        super(GCS21Commands, self).clearparamconv()
    ...
    def _init_valid_ucls(self):
        if not self._settings['validucls']:
        ...
        for block_cmd_line in block_cmd:
            if PIBlockNames.PARAM_OVERVIEW.value in block_cmd_line:
                for line in block_cmd_line[PIBlockNames.PARAM_OVERVIEW.value]:
                    ...
                    elif data_type.upper() in (PIValueDataTypes.STRING32.value, PIValueDataTypes.VOID.value):
                        continue
                    else:
