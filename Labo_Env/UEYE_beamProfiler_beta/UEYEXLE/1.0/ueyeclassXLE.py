...
class UeyeCamera:
    ...
    def configure_device(self, res = '4k', bitDepth = 12):
        """
        Configure camera.
        ...
        try:
            ...
            pass
        if res.lower() == '4k':
            self.__nodemap_remote_device.FindNode('BinningHorizontal').SetValue(1)
            self.imageWidth = 3840
            self.imageHeight = 2160
        else:
            self.__nodemap_remote_device.FindNode('BinningHorizontal').SetValue(2)
            self.imageWidth = 1920
            self.imageHeight = 1080
        self.res = res.lower()
        
        if bitDepth == 12:
            self.__nodemap_remote_device.FindNode('PixelFormat').SetCurrentEntry('Mono12g24IDS')
        else:
            self.__nodemap_remote_device.FindNode('PixelFormat').SetCurrentEntry('Mono10g40IDS')
        self.bitDepth = bitDepth
        
        self.exposure_time_min = self.__nodemap_remote_device.FindNode('ExposureTime').Minimum()
        
        self.exposure_time_max = self.__nodemap_remote_device.FindNode('ExposureTime').Maximum()
        if self.__nodemap_remote_device.FindNode("ExposureTime").HasConstantIncrement():
            self.exposure_time_increment = self.__nodemap_remote_device.FindNode('ExposureTime').Increment()
        else:
            self.exposure_time_increment = ( self.exposure_time_max - self.exposure_time_min) / 100
        
        self.__nodemap_remote_device.FindNode('ExposureTime').SetValue(self.exposure_time_min)
        
        
        
        
        self.framerate_min = self.__nodemap_remote_device.FindNode('AcquisitionFrameRate').Minimum()
        
        self.framerate_max = self.__nodemap_remote_device.FindNode('AcquisitionFrameRate').Maximum()
        
        #self.__nodemap_remote_device.FindNode('AcquisitionFrameRate').SetValue(self.framerate_max)
        try:
    ...
    def get_pixel_format_list(self):
        allEntries = self.__nodemap_remote_device.FindNode("PixelFormat").Entries()
        availableEntries = []
    ...
    def acquire(self):
        ...
        try:
            ...
            if self.bitDepth == 12:
                ...
                array = converted_ipl_image.get_numpy_2D_16()
            elif self.bitDepth == 10:
                converted_ipl_image= ipl_image.ConvertTo(ids_peak_ipl.PixelFormatName_Mono10)
                array = converted_ipl_image.get_numpy_2D_16()
            else:
                converted_ipl_image = ipl_image.ConvertTo(ids_peak_ipl.PixelFormatName_Mono8)
                array = converted_ipl_image.get_numpy_2D()
...
# =============================================================================
    ...
    def set_exposure(self,exposure):
        ...
        """
        self.__nodemap_remote_device.FindNode('ExposureTime').SetValue(exposure*1000)
        return
    ...
    def get_exposure(self):
        ...
        """
        return self.__nodemap_remote_device.FindNode('ExposureTime').Value()/1000
    def get_exposure_min(self):
        """
        Get exposure parameter.
        """
        return self.__nodemap_remote_device.FindNode('ExposureTime').Minimum()/1000
    def get_exposure_max(self):
        """
        Get exposure parameter.
        """
        return self.__nodemap_remote_device.FindNode('ExposureTime').Maximum()/1000
    ...
    def get_pixelSize(self):
        ...
        """
        if self.res == '4k':
            return self.__nodemap_remote_device.FindNode('SensorPixelWidth').Value()
        else:
            return self.__nodemap_remote_device.FindNode('SensorPixelWidth').Value()*2
