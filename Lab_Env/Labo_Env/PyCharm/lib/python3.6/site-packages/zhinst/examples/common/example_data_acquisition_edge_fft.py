...
def run_example(device_id, amplitude=0.25, do_plot=False):
    ...
    # Create an instance of the Data Acquisition Module (ziDAQRecorder class).
    ...
    # The new software trigger module only has grid operation. This means that if you want a plot free
    # of aliasing due to under or over sampling, The grid mode mode must be set accordingly.
    # 1 = Nearest. If the interval between samples on the grid does not match the
    #     interval between demod samples exactly, the nearest sample (in time) is taken.
    # 2 = Linear interpolation. If the interval between samples on the grid does not match the
    #     interval between demod samples exactly, a linear interpolation is done
    #     between the two neighbouring samples.
    # 4 = Exact (on grid). The duration is adjusted so that the interval between
    #     samples on the grid matches exactly the interval between demod samples.
    ...
    if do_plot and samples:
        ...
        for i in range(0, len(samples)):
            bin_count = len(samples[i]['value'][0])
            bin_resolution = samples[i]['header']['gridcoloffset']
            frequencies = np.arange(bin_count)
            # Center frequency and bandwidth not yet implemented. So we calculate from the gridcoloffset.
            bandwidth = bin_resolution*len(frequencies)
            frequencies = frequencies*bin_resolution - bandwidth/2.0 + bin_resolution/2.0
            rDb = 20*np.log10(samples[i]['value'][0]*np.sqrt(2)/amplitude)
