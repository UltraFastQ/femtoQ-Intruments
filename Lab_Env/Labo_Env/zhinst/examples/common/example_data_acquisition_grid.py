...
Python API Example for the Data Acquisition (ziDAQRecorder) Core Module in Grid
...
def run_example(device_id, amplitude=0.25, num_grids=3, do_plot=False):
    ...
    Data Acquisition (ziDAQRecorder) Module in Grid Mode. See this Module's
    ...
    # Start the Data Acquisition's thread. Ready to record triggers.
    trigger.execute()
    ...
    trigger_params = trigger.get('dataAcquisitionModule/*', True)
    timeout = 10  # [s]
    t0 = time.time()
    while trigger_params['/findlevel'] == 1:
        time.sleep(0.05)
        trigger_params = trigger.get('dataAcquisitionModule/*', True)
    ...
    print("SW Trigger found and set dataAcquisitionModule/level: {}, dataAcquisitionModule/hysteresis: {}.".format(
        trigger_params['/level'][0], trigger_params['/hysteresis'][0]))
